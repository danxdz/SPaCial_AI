...
interface WorkstationSelectionProps {
  onNext: () => void;
  onBackToMain?: () => void;
}

const WorkstationSelection = ({ onNext, onBackToMain }: WorkstationSelectionProps) => {
  const { currentUser, setSelectedWorkstation } = useUserStore();
  ...
  }, [currentUser?.selected_workshop_id]);

  const loadWorkstations = async () => {
    try {
      setLoading(true);
      const workstationsData = await db.queryAll(
        'SELECT id, name, description, workshop_id FROM workstations WHERE workshop_id = ? ORDER BY name',
        [currentUser?.selected_workshop_id]
      );
  ...
  const handleWorkstationSelect = (workstationId: number) => {
  ...
  const handleContinue = async () => {
    ...
    await setSelectedWorkstation(selectedWorkstationId);
    onNext();
  };
      ...
      <div className="max-w-md w-full space-y-8">
        <div>
          ...
          <p className="mt-2 text-center text-sm text-gray-600 dark:text-gray-400">
            Choose the workstation/machine you work at
          </p>
        </div>
          ...
          {workstations.length === 0 ? (
            <div className="text-center py-8">
              <p className="text-gray-500 dark:text-gray-400">
                No workstations available for this workshop
              </p>
            </div>
          ) : (
